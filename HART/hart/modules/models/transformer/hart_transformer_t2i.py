...
from hart.modules.networks.fastvar_basic import FastVARAdaLNSelfAttn
...
class HARTForT2I(PreTrainedModel):
    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(self.patch_nums[:-1]):  # [1, 2, 3, 4, 5, 7, 9, 12, 16, 21, 27, 36, 48, 64]
            ...
            x = next_token_map
            FastVARAdaLNSelfAttn.forward

            for layer_idx, b in enumerate(self.blocks): # [24 x blocks]
                x = b(
                    x=x,
            ...
            assert not torch.any(torch.isnan(logits_BlV)), "Output feature map contains NaN"
            ...
            f_hat, next_token_map = self.vae_quant_proxy[0].get_next_autoregressive_input(si, len(self.patch_nums), f_hat, h_BChw, patch_nums=self.patch_nums)
        ...
        for step in indices:
            ...
            # torch.cuda.synchronize()
            # elapsed_time = start_event.elapsed_time(end_event)
            # print('scale:', 64, "total forward running time:", int(elapsed_time), "ms")
            # torch.cuda.empty_cache()
