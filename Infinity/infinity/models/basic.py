...
def precompute_rope2d_freqs_grid(dim, dynamic_resolution_h_w, rope2d_normalized_by_hw, pad_to_multiplier=1, max_height=2048 // 16, max_width=2048 // 16, base=10000.0, device=None, scaling_factor=1.0):
    ...
    for h_div_w in dynamic_resolution_h_w:
        ...
        # # TODO 更大分辨率的图像生成。目前用不到了
        # if h_div_w == 1.0:
        #     scale_schedule.append((1,84,84))
        # if h_div_w == 0.5:
        #     scale_schedule.append((1, 60, 120))
        # if h_div_w == 2.0:
        #     scale_schedule.append((1,120,60))
        # if h_div_w == 1.333:
        #     scale_schedule.append((1,84,63))
...
class CrossAttnBlock(nn.Module):
    ...
    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0,*args,**kwargs):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
